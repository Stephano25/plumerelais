    ...
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        {story?.status !== 'finished' && currentTurn && (
          <View style={styles.timerBadge}><View style={styles.timerDot} /><CountdownTimer endTime={currentTurn.ends_at} onExpire={fetchData} compact={true} /></View>
        )}
        {story?.status === 'finished' && (
          <View style={[styles.timerBadge, styles.finishedBadge]}><Text style={styles.finishedBadgeText}>Terminée</Text></View>
        )}
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          {blindMode && (
            <View style={styles.blindBanner}>
              ...
              <View style={styles.blindContent}>
                ...
                <Text style={styles.blindDesc}>Proposez votre suite pour débloquer l'histoire complète.</Text>
              </View>
            </View>
          )}
          ...
          <View style={styles.paragraphsWrap}>
            {blindMode ? (
              paragraphs.length > 0 && (
                <ParagraphCard p={paragraphs[paragraphs.length - 1]} idx={paragraphs.length - 1} accentColor={authorColor(paragraphs.length - 1)} isLast />
              )
            ) : (
              paragraphs.map((p, idx) => (
                <ParagraphCard key={idx} p={p} idx={idx} accentColor={authorColor(idx)} isLast={idx === paragraphs.length - 1} />
              ))
            )}
          </View>
          ...
          {story?.status !== 'finished' && (
            <View style={styles.infoRow}>
              <Text style={styles.infoText}>{currentTurn ? `Tour ${currentTurn.turn_number}` : 'En attente'}</Text>
              <Text style={styles.infoText}>{paragraphs.length} paragraphe{paragraphs.length !== 1 ? 's' : ''}</Text>
            </View>
          )}
        </Animated.View>
      ...
      <View style={styles.actionBar}>
        ...
        {canPropose() && (
          <TouchableOpacity style={[styles.btn, styles.btnPropose]} onPress={() => navigation.navigate('Propose', { storyId, turnId: currentTurn!.id })} activeOpacity={0.85}>
            <Text style={styles.btnProposeText}>✍️ Proposer une suite</Text>
          </TouchableOpacity>
        )}
        {isParticipant && currentTurn && story?.status !== 'finished' && (
          <TouchableOpacity style={[styles.btn, styles.btnVote]} onPress={() => navigation.navigate('Vote', { storyId, turnId: currentTurn!.id })} activeOpacity={0.85}>
      ...
      <View style={paraStyles.body}>
        ...
        <View style={paraStyles.meta}>
          <View style={[paraStyles.authorDot, { backgroundColor: accentColor }]}>
            <Text style={paraStyles.authorInitial}>{p.author?.username?.[0]?.toUpperCase() || '?'}</Text>
          </View>
          <Text style={paraStyles.authorName}>{p.author?.username}</Text>
          <View style={paraStyles.turnChip}><Text style={paraStyles.turnText}>Tour {p.turn_number}</Text></View>
        </View>
      </View>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingTop: 52, paddingBottom: 14, backgroundColor: colors.bg2, borderBottomWidth: 0.5, borderBottomColor: 'rgba(124,92,191,0.2)' },
  backBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.bg3, borderWidth: 0.5, borderColor: 'rgba(124,92,191,0.3)', alignItems: 'center', justifyContent: 'center' },
  backArrow: { fontSize: 18, color: colors.t1 },
  headerTitle: { flex: 1, fontSize: 16, fontWeight: '600', color: colors.t1 },
  timerBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(232,197,71,0.12)', borderWidth: 0.5, borderColor: 'rgba(232,197,71,0.35)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 },
  timerDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.accent },
  finishedBadge: { backgroundColor: 'rgba(78,203,160,0.12)', borderColor: 'rgba(78,203,160,0.35)' },
  finishedBadgeText: { fontSize: 12, color: colors.green, fontWeight: '600' },
  content: { flex: 1 },
  blindBanner: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', margin: 16, backgroundColor: 'rgba(124,92,191,0.1)', borderWidth: 0.5, borderColor: 'rgba(124,92,191,0.3)', borderRadius: radii.lg, padding: 16 },
  ...
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 8 },
  infoText: { fontSize: 12, color: colors.t3 },
  actionBar: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, gap: 10, backgroundColor: 'rgba(14,11,26,0.97)', borderTopWidth: 0.5, borderTopColor: 'rgba(124,92,191,0.2)' },
  btn: { paddingVertical: 14, borderRadius: radii.md, alignItems: 'center' },
  btnPropose: { backgroundColor: colors.primary, shadowColor: colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 14, elevation: 6 },
  btnProposeText: { color: '#fff', fontSize: 15, fontWeight: '700' },
  btnVote: { backgroundColor: 'rgba(78,203,160,0.1)', borderWidth: 0.5, borderColor: 'rgba(78,203,160,0.35)' },
  btnVoteText: { color: colors.green, fontSize: 15, fontWeight: '600' },
  btnJoin: { backgroundColor: 'rgba(91,174,245,0.1)', borderWidth: 0.5, borderColor: 'rgba(91,174,245,0.35)' },
  btnJoinText: { color: colors.blue, fontSize: 15, fontWeight: '600' },
});

const paraStyles = StyleSheet.create({
  wrap: { flexDirection: 'row', marginBottom: 16, backgroundColor: colors.bg2, borderRadius: 0, borderTopRightRadius: radii.md, borderBottomRightRadius: radii.md, overflow: 'hidden', borderWidth: 0.5, borderLeftWidth: 0, borderColor: 'rgba(124,92,191,0.15)' },
  lastWrap: { borderColor: 'rgba(232,197,71,0.25)' },
  accent: { width: 3, flexShrink: 0 },
  body: { flex: 1, padding: 16 },
  text: { fontSize: 15, lineHeight: 26, color: colors.t1, fontFamily: 'Georgia', fontStyle: 'italic' },
  meta: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 10 },
  authorDot: { width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  authorInitial: { color: '#fff', fontSize: 10, fontWeight: '700' },
  authorName: { fontSize: 12, color: colors.t2, flex: 1 },
  turnChip: { backgroundColor: 'rgba(124,92,191,0.15)', borderRadius: 10, paddingHorizontal: 8, paddingVertical: 2 },
